...
In this section, the methodology of the experiments is presented.
To this end, the material (section \ref{mat}), the individual setups and the overall layout (section \ref{layout}), the transmission method (section \ref{tranMeth}) and the scenarios of the experiments (section \ref{scenario}) are discussed.


\subsection{Material}\label{mat}
The use of material in our experiments can be separated into two parts :

\textbf{The receiver Part} uses a laptop, an USRP \unsure{ unsure of the exact model (B210 or B200?)} Software Defined Radio (SDR) with its enclosure kit and an \unsure{paper antenna brand ?} antenna.

\textbf{The transmitter Part} uses a laptop, 4 StarTech USB hubs (ST7200USBM), 20 Arduino MKR1310 devices with \unsure{plastic antenna brand ?} antenna, a Velleman 5V lab power supply (LABPS3005SM), soldered copper wires to supply the Arduino devices and a handmade frame to attach the Arduino devices to. 

\improvement[inline]{Refer to the transmitter appendix to show the handmade frame}

\subsection{Setups and Layout of the experiments}\label{layout}
...
The method to transmit and receive frames is discussed with two proposed methods :

\textbf{Round-robin method (RR)} : each device sends a single frame then sleeps until all other devices sent their frame. 
This creates a cycle where each device sends one frame and waits for the next cycle.
The method cycles as explained until all frames from each device is sent.
This cycling scheme implies to change from one device sending to another 20 times per cycle.

\textbf{One by One method (ObO)} : each device sends all the frames it has to in a single burst.
A cycle is defined as all the transmissions from a single device. 
The method cycles for each device until all frames from each device is sent.  
This cycling scheme implies to change from one device sending to another only 20 times in total. 


\textbf{Comparison} : the methods are compared by (1) speed, (2) ease of implementation, (3) misclassification and (4) quality.

(1) Each method is similar in terms of speed, though ObO can be faster because there are fewer synchronization constraints.

(2) Both implementations have their specificities, but none are substantially harder to implement then the other.

(3) Misclassification are most likely to occur when changing devices. The number of changes in RR makes it prone to misclassifications. In contrast, ObO has way fewer device changes making it less prone to misclassification.
...
In RR, those variations are present on all devices because a cycle is short. 
On the other hand, in ObO, those variations can appear in the captures because the time between the transmissions of the first and last device is much longer.


\textbf{Selection} : although the quality of the captures from ObO can be lower than in RR, because of time constraints and difficulty to make the Round-robin method work flawlessly, the One by One method is used in the experiments.
...
\begin{table}[ht]
  ...
  \begin{tabular}{|c|c|c|c|}
   \hline 
   $SF$ & $Bw$ (\unit{\kilo\hertz}) & $F_c$ (\unit{\mega\hertz}) & $period$ (\unit{\ms}) \\
   \hline 
   $7$ & $125$ & $868$ & $200$ \\
   \hline 
   $frames\ per\ device$ & $number\ of\ devices$ & $Gain$ (\unit{\decibel}) & $F_s$ (\unit{samples\per\s}) \\
   \hline 
   $550$ & $20$ & $+40$ & $10^6$ \\
   \hline 
  \end{tabular}
...
\textbf{Inside} : The sports hall in the Vésale building on the Nimy Campus at Umons is used as the inside environment.
...
\textbf{Outside} : The grass field between the DeVinci building and the "bâtiment 4" on the Nimy Campus at Umons is used as the outside environment. 
...
\unsure[inline]{I'm unsure on how to locate the environments}
