...
class SerialController:
    ...
    def reset_error(self, signum, frame):
        print(
            f"{bcolors.FAIL}Error reseting of device {self.device_id} on port {self.port}{bcolors.ENDC}"
        )
        self.reset()
...
def scheduleSending(controller_list, starting_time, period, cycles):
    ...
    wait_until(starting_time)

    sleep = starting_time + period
...
if __name__ == "__main__":
    ...
    controller_list = get_serial_controllers(
