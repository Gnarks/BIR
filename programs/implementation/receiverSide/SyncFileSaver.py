...
class SyncFileSaver(gr.sync_block):
    ...
    def __init__(
        ...
        gr.sync_block.__init__(
            self, name="Syncronised File Saver", in_sig=[np.complex64], out_sig=None
        )
    ...
    def work(self, input_items, output_items):
        ...
        #    print(f"next received: {self.next} compared to current {time.time()}")
        #    self.first = False
    ...
    def save_frame_during_period(self, data):
        ...
        print(
            f"stopped saving at {time.time()} compared to {self.next} \n drift : {time.time() - self.next}"
        )
        ...
        # save the meta file
        meta.tofile(
            f"{self.save_directory}/{self.scenario_name}/device{self.current_device}/cycle_{self.current_cycle}.sigmf-meta"
        )  # extension is optional

        print(
            f"{self.current_device} : [{self.current_cycle}/{self.cycles}] end saved at {time.time()}"
        )
        self.firstt = True
        return True
