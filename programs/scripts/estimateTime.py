...
number_of_frame_per_device = int(
    input("number_of_frame_per_device (default=1000): ").strip() or "1000"
)
...
single_frame_transimttion_time = round(
    (preambule_size + 4.25) * (math.pow(2, spreding_factor) / bandwith) * 1000, 2
)

full_cycle_of_transmittion = round(
    (single_frame_transimttion_time + time_between_frame) * number_of_device, 4
)
